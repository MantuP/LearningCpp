###############################################################################################
#File name	: Ittis_IT1_Manipulation.py
#Author		: Prabina Pradhan
#Date		: 06/05/2022
#Purpose	: To convert Ittis_IT1_ partner tariff sheet into TT WBS format.
#
#
###############################################################################################
...
def readAndManipulation(p_file,p_uploudPrefix,p_uploudExtension,p_readFilePath,p_errorFilePath,p_upoladededPath,p_bkp_downloadedPath,p_OutPutColumnsHeader,p_OutPutColumnsHeaderDelm):
	print("Call Ittis_IT1_Manipulation.py.", p_file)
	logging.info("Called Ittis_IT1_Manipulation.py for reading %s file and converting into TT WBS tariff upload formate.", p_file)
	...
	l_NumberOfSheetInFileVal = Utility.getConfValueUsingLevel('l_NumberOfSheetInFile','ITTIS')
	...
	l_ExcelFileNameList_Val = Utility.getConfValueUsingLevel('l_ExcelFileNameList','ITTIS')
	s1 = Utility.listToStringWithoutBracketsSinglequote(getTheSheetNames_list)
	s2 = l_ExcelFileNameList_Val
	s1 = Utility.remove_end_spaces(s1)
	s2 = Utility.remove_end_spaces(s2)
	...
	l_getCanarSheetName = Utility.getConfValueUsingLevel('l_SheetNameIndex','ITTIS')
	...
	l_getCurrencyVal = Utility.getConfValueUsingLevel('l_CurrencyFromConf','ITTIS')
	#print(l_getCurrencyVal)
	
	#Get the reading start position
	l_getRadingStartPosRow = Utility.getConfValueUsingLevel('l_RadingStartPosRow','ITTIS')
	...
	#l_getSearchKeyWord = Utility.getConfValueUsingLevel('l_SearchKeyWordEndPos','ITTIS')
	#l_getSearchKeyWordGap = Utility.getConfValueUsingLevel('l_SearchKeyWordEndPosGap','ITTIS')
	...
	l_DialCodePosVal = Utility.getConfValueUsingLevel('l_DialCodePos','ITTIS')
	...
	l_CheckFirstSepratorVal = Utility.getConfValueUsingLevel('l_CheckFirstSeprator','ITTIS')
	l_CheckSecondSepratorVal = Utility.getConfValueUsingLevel('l_CheckSecondSeprator','ITTIS')
	l_CheckFirstSepratorManualyVal = Utility.getConfValueUsingLevel('l_CheckFirstSepratorManualy','ITTIS')
	...
	l_EffectiveEndDateVal =  Utility.getConfValueUsingLevel('l_EffectiveEndDate','ITTIS')
	l_EffectiveEndDateValDate = UtilityForExcelFile.getConvertNumberToDateInDDMMYYYYHH24MISS(l_EffectiveEndDateVal)
	
	l_FetchColumnSequence1Val = Utility.getConfValueUsingLevel('l_FetchColumnSequence1','ITTIS')
	l_FetchColumnSequence2Val = Utility.getConfValueUsingLevel('l_FetchColumnSequence2','ITTIS')
	l_FetchColumnSequence3Val = Utility.getConfValueUsingLevel('l_FetchColumnSequence3','ITTIS')
	l_FetchColumnSequence4Val = Utility.getConfValueUsingLevel('l_FetchColumnSequence4','ITTIS')
	#l_FetchColumnSequence5Val = Utility.getConfValueUsingLevel('l_FetchColumnSequence5','ITTIS')
	...
	l_PtrnerNameSl					= str(Utility.getConfValueUsingLevel('l_PtrnerName','ITTIS'))
	...
	l_dropFromTailOfDataFreamVal	= str(Utility.getConfValueUsingLevel('l_dropFromTailOfDataFream','ITTIS'))
	...
	l_PresentDifferSeparatorInUnitRateVal	= str(Utility.getConfValueUsingLevel('l_PresentDifferSeparatorInUnitRate','ITTIS'))
	if int(l_PresentDifferSeparatorInUnitRateVal) == l_One:
		l_PresentSeparatorInUnitRateVal	= str(Utility.getConfValueUsingLevel('l_PresentSeparatorInUnitRate','ITTIS'))
