###############################################################################################
#File name	: Etiuae_ET2_Manipulation.py
#Author		: Prabina Pradhan
#Date		: 26/04/2022
#Purpose	: To convert Etiuae_ET2_ partner tariff sheet into TT WBS format.
# 1. Empty Excel sheet
# 2. Added data_only for coping the data not formula
###############################################################################################
...
def readAndManipulation(p_file,p_uploudPrefix,p_uploudExtension,p_readFilePath,p_errorFilePath,p_upoladededPath,p_bkp_downloadedPath,p_OutPutColumnsHeader,p_OutPutColumnsHeaderDelm):
	print("Call Etiuae_ET2_Manipulation.py.", p_file)
	logging.info("Called Etiuae_ET2_Manipulation.py for reading %s file and converting into TT WBS tariff upload formate.", p_file)
	...
	l_NumberOfSheetInFileVal = Utility.getConfValueUsingLevel('l_NumberOfSheetInFile','ETIUAE')
	#print(l_Rd_file_SheetCnt);
	#print(l_NumberOfSheetInFileVal);
	...
	l_ExcelFileNameList_Val = Utility.getConfValueUsingLevel('l_ExcelFileNameList','ETIUAE')
	...
	l_getCanarSheetName = Utility.getConfValueUsingLevel('l_SheetNameIndex','ETIUAE')
	...
	#print(l_Rd_file_indexUsingShtName)
	
	l_getCurrencyVal = Utility.getConfValueUsingLevel('l_CurrencyFromConf','ETIUAE')
	#print(l_getCurrencyVal)
	
	#Get the reading start position
	l_getRadingStartPosRow = Utility.getConfValueUsingLevel('l_RadingStartPosRow','ETIUAE')
	...
	l_getSearchKeyWord = Utility.getConfValueUsingLevel('l_SearchKeyWordEndPos','ETIUAE')
	l_getSearchKeyWordGap = Utility.getConfValueUsingLevel('l_SearchKeyWordEndPosGap','ETIUAE')
	...
	#print(l_getSearchEndPosRow[l_One]);
	#print(l_getReadingEndPosRow)

	# Get actual values in list
	l_DialCodePosVal = Utility.getConfValueUsingLevel('l_DialCodePos','ETIUAE')
	...
	l_CheckFirstSepratorVal = Utility.getConfValueUsingLevel('l_CheckFirstSeprator','ETIUAE')
	l_CheckSecondSepratorVal = Utility.getConfValueUsingLevel('l_CheckSecondSeprator','ETIUAE')
	l_CheckFirstSepratorManualyVal = Utility.getConfValueUsingLevel('l_CheckFirstSepratorManualy','ETIUAE')
	...
	if Utility.listToCheckSymbolPresentOrNotStr(l_DialCodePosList,l_CheckFirstSepratorVal) == l_One:
	...
	l_EffectiveEndDateVal =  Utility.getConfValueUsingLevel('l_EffectiveEndDate','ETIUAE')
	l_EffectiveEndDateValDate = UtilityForExcelFile.getConvertNumberToDateInDDMMYYYYHH24MISS(l_EffectiveEndDateVal)
	
	l_FetchColumnSequence1Val = Utility.getConfValueUsingLevel('l_FetchColumnSequence1','ETIUAE')
	l_FetchColumnSequence2Val = Utility.getConfValueUsingLevel('l_FetchColumnSequence2','ETIUAE')
	l_FetchColumnSequence3Val = Utility.getConfValueUsingLevel('l_FetchColumnSequence3','ETIUAE')
	l_FetchColumnSequence4Val = Utility.getConfValueUsingLevel('l_FetchColumnSequence4','ETIUAE')
	l_FetchColumnSequence5Val = Utility.getConfValueUsingLevel('l_FetchColumnSequence5','ETIUAE')
	...
	l_PtrnerNameSl					= str(Utility.getConfValueUsingLevel('l_PtrnerName','ETIUAE'))
	...
	#excel_data_df = pandas.read_excel(p_readFilePath + p_file, sheet_name = int(l_getCanarSheetName), header = int(l_getRadingStartPosRow) - 0, index_col=None, usecols = w_l)
	excel_data_df = pandas.read_excel(p_readFilePath + p_file, sheet_name = int(l_getCanarSheetName), header = int(l_getRadingStartPosRow) - 0, index_col=None, usecols = w_l,skipfooter=3 )

	excel_data_df.set_axis (['Destination_Description', 'Destination_Number', 'Unit_Rate', 'Rate_Change_Indicator','Start_Date'], axis = 1, inplace = True)
	excel_data_df.head()
	...
	#l_dropFromTailOfDataFreamVal	= str(Utility.getConfValueUsingLevel('l_dropFromTailOfDataFream','ETIUAE'))
	...
	#print(excel_data_df['Start_Date'])
	#print(excel_data_df)
	...
	l_PresentDifferSeparatorInUnitRateVal	= str(Utility.getConfValueUsingLevel('l_PresentDifferSeparatorInUnitRate','ETIUAE'))
	if int(l_PresentDifferSeparatorInUnitRateVal) == l_One:
		l_PresentSeparatorInUnitRateVal	= str(Utility.getConfValueUsingLevel('l_PresentSeparatorInUnitRate','ETIUAE'))
