###############################################################################################
#File name	: Liquid_LI1_Manipulation.py
#Author		: Prabina Pradhan
#Date		: 27/04/2022
#Purpose	: To convert Liquid_LI1_ partner tariff sheet into TT WBS format.
#
#
###############################################################################################
...
def readAndManipulation(p_file,p_uploudPrefix,p_uploudExtension,p_readFilePath,p_errorFilePath,p_upoladededPath,p_bkp_downloadedPath,p_OutPutColumnsHeader,p_OutPutColumnsHeaderDelm):
	print("Call Liquid_LI1_Manipulation.py..............", p_file)
	logging.info("Called Liquid_LI1_Manipulation.py for reading %s file and converting into TT WBS tariff upload formate.", p_file)
	...
	l_NumberOfSheetInFileVal = Utility.getConfValueUsingLevel('l_NumberOfSheetInFile','LIQUID')
	...
	l_ExcelFileNameList_Val = Utility.getConfValueUsingLevel('l_ExcelFileNameList','LIQUID')
	...
	l_getCanarSheetName = Utility.getConfValueUsingLevel('l_SheetNameIndex','LIQUID')
	...
	l_getCurrencyVal = Utility.getConfValueUsingLevel('l_CurrencyFromConf','LIQUID')
	#print(l_getCurrencyVal)
	
	#Get the reading start position
	l_getRadingStartPosRow = Utility.getConfValueUsingLevel('l_RadingStartPosRow','LIQUID')
	...
	l_getSearchKeyWord = Utility.getConfValueUsingLevel('l_SearchKeyWordEndPos','LIQUID')
	l_getSearchKeyWordGap = Utility.getConfValueUsingLevel('l_SearchKeyWordEndPosGap','LIQUID')
	...
	l_DialCodePosVal = Utility.getConfValueUsingLevel('l_DialCodePos','LIQUID')
	...
	#print(l_DialCodePosList); print(len(l_DialCodePosList)); print(type(l_DialCodePosList))
	
	l_CheckFirstSepratorVal = Utility.getConfValueUsingLevel('l_CheckFirstSeprator','LIQUID')
	l_CheckSecondSepratorVal = Utility.getConfValueUsingLevel('l_CheckSecondSeprator','LIQUID')
	l_CheckFirstSepratorManualyVal = Utility.getConfValueUsingLevel('l_CheckFirstSepratorManualy','LIQUID')
	...
	if Utility.listToCheckSymbolPresentOrNotStr(l_DialCodePosList,l_CheckSecondSepratorVal) == l_One and l_CheckFirstSepratorManualyVal == l_One:
		l_loopVar = l_loopVar + l_One
	#print("l_loopVar ",l_loopVar)
	
	l_EffectiveEndDateVal =  Utility.getConfValueUsingLevel('l_EffectiveEndDate','LIQUID')
	l_EffectiveEndDateValDate = UtilityForExcelFile.getConvertNumberToDateInDDMMYYYYHH24MISS(l_EffectiveEndDateVal)
	
	l_FetchColumnSequence1Val = Utility.getConfValueUsingLevel('l_FetchColumnSequence1','LIQUID')
	l_FetchColumnSequence2Val = Utility.getConfValueUsingLevel('l_FetchColumnSequence2','LIQUID')
	l_FetchColumnSequence3Val = Utility.getConfValueUsingLevel('l_FetchColumnSequence3','LIQUID')
	l_FetchColumnSequence4Val = Utility.getConfValueUsingLevel('l_FetchColumnSequence4','LIQUID')
	l_FetchColumnSequence5Val = Utility.getConfValueUsingLevel('l_FetchColumnSequence5','LIQUID')
	...
	l_PtrnerNameSl					= str(Utility.getConfValueUsingLevel('l_PtrnerName','LIQUID'))
	...
	excel_data_df.set_axis (['Destination_Description', 'Destination_Number', 'Start_Date', 'Rate_Change_Indicator','Unit_Rate'], axis = 1, inplace = True)
	excel_data_df.head()
	...
	l_dropFromTailOfDataFreamVal	= str(Utility.getConfValueUsingLevel('l_dropFromTailOfDataFream','LIQUID'))
	...
	excel_data_df['Destination_Number'] = excel_data_df['Destination_Number'].astype(str).apply(lambda x: x.replace('.0',''))
	excel_data_df.head(l_getReadingEndPosRow - l_getRadingStartPosRow).to_csv(l_fullFileName, sep ='|', index = False ,header = True )
	#print(excel_data_df)
	
	l_PresentDifferSeparatorInUnitRateVal	= str(Utility.getConfValueUsingLevel('l_PresentDifferSeparatorInUnitRate','LIQUID'))
	if int(l_PresentDifferSeparatorInUnitRateVal) == l_One:
		l_PresentSeparatorInUnitRateVal	= str(Utility.getConfValueUsingLevel('l_PresentSeparatorInUnitRate','LIQUID'))
		excel_data_df["Unit_Rate"]=excel_data_df["Unit_Rate"].str.replace(l_PresentSeparatorInUnitRateVal,'.')
		excel_data_df.head(l_getReadingEndPosRow - l_getRadingStartPosRow).to_csv(l_fullFileName, sep ='|', index = False ,header = True )
	
	if l_loopVar == 1:
		excel_data_df["Destination_Number"]=excel_data_df["Destination_Number"].str.split(",")
		#excel_data_df["Destination_Number"]=excel_data_df["Destination_Number"].str.replace('.0','')
