###############################################################################################
#File name	: Orange_OR1_Manipulation.py
...
def readAndManipulation(p_file,p_uploudPrefix,p_uploudExtension,p_readFilePath,p_errorFilePath,p_upoladededPath,p_bkp_downloadedPath,p_OutPutColumnsHeader,p_OutPutColumnsHeaderDelm):
	print("Call Orange_OR1_Manipulation.py.", p_file)
	logging.info("Called Orange_OR1_Manipulation.py for reading %s file and converting into TT WBS tariff upload formate.", p_file)
	...
	l_NumberOfSheetInFileVal = Utility.getConfValueUsingLevel('l_NumberOfSheetInFile','ORANGE')
	...
	l_ExcelFileNameList_Val = Utility.getConfValueUsingLevel('l_ExcelFileNameList','ORANGE')
	...
	l_getCanarSheetName = Utility.getConfValueUsingLevel('l_SheetNameIndex','ORANGE')
	...
	l_getCurrencyVal = Utility.getConfValueUsingLevel('l_CurrencyFromConf','ORANGE')
	#print(l_getCurrencyVal)
	
	#Get the reading start position
	l_getRadingStartPosRow = Utility.getConfValueUsingLevel('l_RadingStartPosRow','ORANGE')
	...
	l_getSearchKeyWord = Utility.getConfValueUsingLevel('l_SearchKeyWordEndPos','ORANGE')
	l_getSearchKeyWordGap = Utility.getConfValueUsingLevel('l_SearchKeyWordEndPosGap','ORANGE')
	...
	l_DialCodePosVal = Utility.getConfValueUsingLevel('l_DialCodePos','ORANGE')
	...
	l_CheckFirstSepratorVal = Utility.getConfValueUsingLevel('l_CheckFirstSeprator','ORANGE')
	l_CheckSecondSepratorVal = Utility.getConfValueUsingLevel('l_CheckSecondSeprator','ORANGE')
	...
	l_EffectiveEndDateVal =  Utility.getConfValueUsingLevel('l_EffectiveEndDate','ORANGE')
	l_EffectiveEndDateValDate = UtilityForExcelFile.getConvertNumberToDateInDDMMYYYYHH24MISS(l_EffectiveEndDateVal)
	#print(l_EffectiveEndDateValDate)
	
	l_FetchColumnSequence1Val = Utility.getConfValueUsingLevel('l_FetchColumnSequence1','ORANGE')
	l_FetchColumnSequence2Val = Utility.getConfValueUsingLevel('l_FetchColumnSequence2','ORANGE')
	l_FetchColumnSequence3Val = Utility.getConfValueUsingLevel('l_FetchColumnSequence3','ORANGE')
	l_FetchColumnSequence4Val = Utility.getConfValueUsingLevel('l_FetchColumnSequence4','ORANGE')
	l_FetchColumnSequence5Val = Utility.getConfValueUsingLevel('l_FetchColumnSequence5','ORANGE')
	...
	l_PtrnerNameSl					= str(Utility.getConfValueUsingLevel('l_PtrnerName','ORANGE'))
	...
	excel_data_df = pandas.read_excel(p_readFilePath + p_file, sheet_name = int(l_getCanarSheetName), header = int(l_getRadingStartPosRow) - 2, index_col=None, usecols = w_l, skipfooter=2 )
	...
	#excel_data_df.set_axis (['Destination_Description', 'Destination_Number', 'Start_Date', 'Unit_Rate','Rate_Change_Indicator'], axis = 1)
	excel_data_df.head()
	...
	l_dropFromTailOfDataFreamVal	= str(Utility.getConfValueUsingLevel('l_dropFromTailOfDataFream','ORANGE'))
	...
	l_PresentDifferSeparatorInUnitRateVal	= str(Utility.getConfValueUsingLevel('l_PresentDifferSeparatorInUnitRate','ORANGE'))
	if int(l_PresentDifferSeparatorInUnitRateVal) == l_One:
		l_PresentSeparatorInUnitRateVal	= str(Utility.getConfValueUsingLevel('l_PresentSeparatorInUnitRate','ORANGE'))
		excel_data_df["Unit_Rate"]=excel_data_df["Unit_Rate"].str.replace(l_PresentSeparatorInUnitRateVal,'.')	
		excel_data_df.head(l_getReadingEndPosRow - l_getRadingStartPosRow).to_csv(l_fullFileName, sep ='|', index = False ,header = True )		
		#print(excel_data_df)
	
	if l_loopVar == 1:
		#excel_data_df["Destination_Number"] = excel_data_df["Destination_Number"].astype(str)
