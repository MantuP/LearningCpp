###############################################################################################
#File name	: Telco_TE1_Manipulation.py
#Author		: Prabina Pradhan
#Date		: 02/05/2022
#Purpose	: To convert Telco_TE1_ partner tariff sheet into TT WBS format.
#
#
###############################################################################################
...
def readAndManipulation(p_file,p_uploudPrefix,p_uploudExtension,p_readFilePath,p_errorFilePath,p_upoladededPath,p_bkp_downloadedPath,p_OutPutColumnsHeader,p_OutPutColumnsHeaderDelm):
	print("Call Telco_TE1_Manipulation.py.", p_file)
	logging.info("Called Telco_TE1_Manipulation.py for reading %s file and converting into TT WBS tariff upload formate.", p_file)
	...
	l_NumberOfSheetInFileVal = Utility.getConfValueUsingLevel('l_NumberOfSheetInFile','TELCO')
	...
	l_ExcelFileNameList_Val = Utility.getConfValueUsingLevel('l_ExcelFileNameList','TELCO')
	...
	l_getCanarSheetName = Utility.getConfValueUsingLevel('l_SheetNameIndex','TELCO')
	...
	l_getCurrencyVal = Utility.getConfValueUsingLevel('l_CurrencyFromConf','TELCO')
	#print(l_getCurrencyVal)
	
	#Get the reading start position
	l_getRadingStartPosRow = Utility.getConfValueUsingLevel('l_RadingStartPosRow','TELCO')
	...
	l_getSearchKeyWord = Utility.getConfValueUsingLevel('l_SearchKeyWordEndPos','TELCO')
	l_getSearchKeyWordGap = Utility.getConfValueUsingLevel('l_SearchKeyWordEndPosGap','TELCO')
	...
	#print(l_getSearchEndPosRow[l_One]); print("MaxRow",l_getReadingEndPosRow)

	# Get actual values in list
	l_DialCodePosVal = Utility.getConfValueUsingLevel('l_DialCodePos','TELCO')
	...
	l_CheckFirstSepratorVal = Utility.getConfValueUsingLevel('l_CheckFirstSeprator','TELCO')
	l_CheckSecondSepratorVal = Utility.getConfValueUsingLevel('l_CheckSecondSeprator','TELCO')
	l_CheckFirstSepratorManualyVal = Utility.getConfValueUsingLevel('l_CheckFirstSepratorManualy','TELCO')
	...
	l_EffectiveEndDateVal =  Utility.getConfValueUsingLevel('l_EffectiveEndDate','TELCO')
	l_EffectiveEndDateValDate = UtilityForExcelFile.getConvertNumberToDateInDDMMYYYYHH24MISS(l_EffectiveEndDateVal)
	
	l_FetchColumnSequence1Val = Utility.getConfValueUsingLevel('l_FetchColumnSequence1','TELCO')
	l_FetchColumnSequence2Val = Utility.getConfValueUsingLevel('l_FetchColumnSequence2','TELCO')
	l_FetchColumnSequence3Val = Utility.getConfValueUsingLevel('l_FetchColumnSequence3','TELCO')
	l_FetchColumnSequence4Val = Utility.getConfValueUsingLevel('l_FetchColumnSequence4','TELCO')
	#l_FetchColumnSequence5Val = Utility.getConfValueUsingLevel('l_FetchColumnSequence5','TELCO')
	...
	w_l = f"{l_FetchColumnSequence1Val},{l_FetchColumnSequence2Val},{l_FetchColumnSequence3Val},{l_FetchColumnSequence4Val}"  
	
	l_Rate_Change_IndicatorSl		= Utility.getConfValueUsingLevel('Rate_Change_Indicator')
	...
	l_PtrnerNameSl					= str(Utility.getConfValueUsingLevel('l_PtrnerName','TELCO'))
	...
	excel_data_df.set_axis (['Destination_Description', 'Destination_Number', 'Unit_Rate', 'Start_Date'], axis = 1, inplace = True)
	excel_data_df.head()
	excel_data_df = excel_data_df.reindex(['Destination_Description','Destination_Number','Unit_Rate','Start_Date'], axis=1)
	excel_data_df.insert(4, 'Rate_Change_Indicator', l_Rate_Change_IndicatorSl, True)
	...
	l_dropFromTailOfDataFreamVal	= str(Utility.getConfValueUsingLevel('l_dropFromTailOfDataFream','TELCO'))
	...
	l_PresentDifferSeparatorInUnitRateVal	= str(Utility.getConfValueUsingLevel('l_PresentDifferSeparatorInUnitRate','TELCO'))
	if int(l_PresentDifferSeparatorInUnitRateVal) == l_One:
		l_PresentSeparatorInUnitRateVal	= str(Utility.getConfValueUsingLevel('l_PresentSeparatorInUnitRate','TELCO'))
