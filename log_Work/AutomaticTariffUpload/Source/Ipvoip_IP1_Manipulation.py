###############################################################################################
#File name	: Ipvoip_IP1_Manipulation.py
#Author		: Prabina Pradhan
#Date		: 27/04/2022
#Purpose	: To convert Ipvoip_IP1_ partner tariff sheet into TT WBS format.
#
#
###############################################################################################
...
def readAndManipulation(p_file,p_uploudPrefix,p_uploudExtension,p_readFilePath,p_errorFilePath,p_upoladededPath,p_bkp_downloadedPath,p_OutPutColumnsHeader,p_OutPutColumnsHeaderDelm):
	print("Call Ipvoip_IP1_Manipulation.py.", p_file)
	logging.info("Called Ipvoip_IP1_Manipulation.py for reading %s file and converting into TT WBS tariff upload formate.", p_file)
	...
	l_NumberOfSheetInFileVal = Utility.getConfValueUsingLevel('l_NumberOfSheetInFile','IPVOIP')
	...
	l_ExcelFileNameList_Val = Utility.getConfValueUsingLevel('l_ExcelFileNameList','IPVOIP')
	...
	l_getCanarSheetName = Utility.getConfValueUsingLevel('l_SheetNameIndex','IPVOIP')
	...
	l_getCurrencyVal = Utility.getConfValueUsingLevel('l_CurrencyFromConf','IPVOIP')
	#print(l_getCurrencyVal)
	
	#Get the reading start position
	l_getRadingStartPosRow = Utility.getConfValueUsingLevel('l_RadingStartPosRow','IPVOIP')
	...
	#l_getSearchKeyWord = Utility.getConfValueUsingLevel('l_SearchKeyWordEndPos','IPVOIP')
	#l_getSearchKeyWordGap = Utility.getConfValueUsingLevel('l_SearchKeyWordEndPosGap','IPVOIP')
	...
	l_DialCodePosVal = Utility.getConfValueUsingLevel('l_DialCodePos','IPVOIP')
	...
	l_CheckFirstSepratorVal = Utility.getConfValueUsingLevel('l_CheckFirstSeprator','IPVOIP')
	l_CheckSecondSepratorVal = Utility.getConfValueUsingLevel('l_CheckSecondSeprator','IPVOIP')
	l_CheckFirstSepratorManualyVal = Utility.getConfValueUsingLevel('l_CheckFirstSepratorManualy','IPVOIP')
	...
	l_EffectiveEndDateVal =  Utility.getConfValueUsingLevel('l_EffectiveEndDate','IPVOIP')
	l_EffectiveEndDateValDate = UtilityForExcelFile.getConvertNumberToDateInDDMMYYYYHH24MISS(l_EffectiveEndDateVal)
	
	l_FetchColumnSequence1Val = Utility.getConfValueUsingLevel('l_FetchColumnSequence1','IPVOIP')
	l_FetchColumnSequence2Val = Utility.getConfValueUsingLevel('l_FetchColumnSequence2','IPVOIP')
	l_FetchColumnSequence3Val = Utility.getConfValueUsingLevel('l_FetchColumnSequence3','IPVOIP')
	l_FetchColumnSequence4Val = Utility.getConfValueUsingLevel('l_FetchColumnSequence4','IPVOIP')
	#l_FetchColumnSequence5Val = Utility.getConfValueUsingLevel('l_FetchColumnSequence5','IPVOIP')
	...
	l_PtrnerNameSl					= str(Utility.getConfValueUsingLevel('l_PtrnerName','IPVOIP'))
	...
	excel_data_df.set_axis (['Destination_Number', 'Destination_Description', 'Unit_Rate', 'Start_Date'], axis = 1, inplace = True)
	excel_data_df.head()
	...
	l_dropFromTailOfDataFreamVal	= str(Utility.getConfValueUsingLevel('l_dropFromTailOfDataFream','IPVOIP'))
	...
	l_PresentDifferSeparatorInUnitRateVal	= str(Utility.getConfValueUsingLevel('l_PresentDifferSeparatorInUnitRate','IPVOIP'))
	if int(l_PresentDifferSeparatorInUnitRateVal) == l_One:
		l_PresentSeparatorInUnitRateVal	= str(Utility.getConfValueUsingLevel('l_PresentSeparatorInUnitRate','IPVOIP'))
