###############################################################################################
#File name	: Pccw_PC1_Manipulation.py
#Author		: Prabina Pradhan
#Date		: 02/05/2022
#Purpose	: To convert Pccw_PC1_ partner tariff sheet into TT WBS format.
#
#
###############################################################################################
...
def readAndManipulation(p_file,p_uploudPrefix,p_uploudExtension,p_readFilePath,p_errorFilePath,p_upoladededPath,p_bkp_downloadedPath,p_OutPutColumnsHeader,p_OutPutColumnsHeaderDelm):
	print("Call Pccw_PC1_Manipulation.py.", p_file)
	logging.info("Called Pccw_PC1_Manipulation.py for reading %s file and converting into TT WBS tariff upload formate.", p_file)
	...
	l_NumberOfSheetInFileVal = Utility.getConfValueUsingLevel('l_NumberOfSheetInFile','PCCW')
	...
	l_ExcelFileNameList_Val = Utility.getConfValueUsingLevel('l_ExcelFileNameList','PCCW')
	...
	l_getCanarSheetName = Utility.getConfValueUsingLevel('l_SheetNameIndex','PCCW')
	...
	l_getCurrencyVal = Utility.getConfValueUsingLevel('l_CurrencyFromConf','PCCW')
	#print(l_getCurrencyVal)
	
	#Get the reading start position
	l_getRadingStartPosRow = Utility.getConfValueUsingLevel('l_RadingStartPosRow','PCCW')
	...
	#l_getSearchKeyWord = Utility.getConfValueUsingLevel('l_SearchKeyWordEndPos','PCCW')
	#l_getSearchKeyWordGap = Utility.getConfValueUsingLevel('l_SearchKeyWordEndPosGap','PCCW')
	...
	l_DialCodePosVal = Utility.getConfValueUsingLevel('l_DialCodePos','PCCW')
	...
	l_CheckFirstSepratorVal = Utility.getConfValueUsingLevel('l_CheckFirstSeprator','PCCW')
	l_CheckSecondSepratorVal = Utility.getConfValueUsingLevel('l_CheckSecondSeprator','PCCW')
	l_CheckFirstSepratorManualyVal = Utility.getConfValueUsingLevel('l_CheckFirstSepratorManualy','PCCW')
	#print(l_CheckFirstSepratorVal) 
	#print(l_CheckSecondSepratorVal)
	#print(l_CheckFirstSepratorManualyVal)
	#print(int(Utility.listToCheckSymbolPresentOrNotStr(l_DialCodePosList,l_CheckFirstSepratorVal)))
	...
	l_EffectiveEndDateVal =  Utility.getConfValueUsingLevel('l_EffectiveEndDate','PCCW')
	l_EffectiveEndDateValDate = UtilityForExcelFile.getConvertNumberToDateInDDMMYYYYHH24MISS(l_EffectiveEndDateVal)
	
	l_FetchColumnSequence1Val = Utility.getConfValueUsingLevel('l_FetchColumnSequence1','PCCW')
	l_FetchColumnSequence2Val = Utility.getConfValueUsingLevel('l_FetchColumnSequence2','PCCW')
	l_FetchColumnSequence3Val = Utility.getConfValueUsingLevel('l_FetchColumnSequence3','PCCW')
	l_FetchColumnSequence4Val = Utility.getConfValueUsingLevel('l_FetchColumnSequence4','PCCW')
	l_FetchColumnSequence5Val = Utility.getConfValueUsingLevel('l_FetchColumnSequence5','PCCW')
	...
	l_PtrnerNameSl					= str(Utility.getConfValueUsingLevel('l_PtrnerName','PCCW'))
	...
	l_dropFromTailOfDataFreamVal	= str(Utility.getConfValueUsingLevel('l_dropFromTailOfDataFream','PCCW'))
	...
	excel_data_df["Unit_Rate"] = excel_data_df["Unit_Rate"].apply(replace_non_numeric_with_zero)
	
	excel_data_df.head(l_getReadingEndPosRow - (l_getRadingStartPosRow - 1)).to_csv(l_fullFileName, sep ='|', index = False ,header = True )
	#print(excel_data_df)
	
	l_PresentDifferSeparatorInUnitRateVal	= str(Utility.getConfValueUsingLevel('l_PresentDifferSeparatorInUnitRate','PCCW'))
	if int(l_PresentDifferSeparatorInUnitRateVal) == l_One:
		l_PresentSeparatorInUnitRateVal	= str(Utility.getConfValueUsingLevel('l_PresentSeparatorInUnitRate','PCCW'))
