###############################################################################################
#File name	: Apelby_AP1_Manipulation.py
#Author		: Prabina Pradhan
#Date		: 06/05/2022
#Purpose	: To convert Apelby_AP1_ partner tariff sheet into TT WBS format.
#
#
###############################################################################################
...
def readAndManipulation(p_file,p_uploudPrefix,p_uploudExtension,p_readFilePath,p_errorFilePath,p_upoladededPath,p_bkp_downloadedPath,p_OutPutColumnsHeader,p_OutPutColumnsHeaderDelm):
	print("Call Apelby_AP1_Manipulation.py ---------", p_file)
	logging.info("Called Apelby_AP1_Manipulation.py for reading %s file and converting into TT WBS tariff upload formate.", p_file)
	...
	l_NumberOfSheetInFileVal = Utility.getConfValueUsingLevel('l_NumberOfSheetInFile','APELBY')
	...
	l_ExcelFileNameList_Val = Utility.getConfValueUsingLevel('l_ExcelFileNameList','APELBY')
	...
	l_getCanarSheetName = Utility.getConfValueUsingLevel('l_SheetNameIndex','APELBY')
	...
	l_getCurrencyVal = Utility.getConfValueUsingLevel('l_CurrencyFromConf','APELBY')
	#print(l_getCurrencyVal)
	
	#Get the reading start position
	l_getRadingStartPosRow = Utility.getConfValueUsingLevel('l_RadingStartPosRow','APELBY')
	...
	#l_getSearchKeyWord = Utility.getConfValueUsingLevel('l_SearchKeyWordEndPos','APELBY')
	#l_getSearchKeyWordGap = Utility.getConfValueUsingLevel('l_SearchKeyWordEndPosGap','APELBY')
	#l_getSearchEndPosRow = UtilityForExcelFile.getIdUsingSearchValueInColumn(l_Rd_file_indexUsingShtName,l_getSearchKeyWord)
	#l_getReadingEndPosRow = int(l_getSearchEndPosRow[l_One]) - (int(l_getSearchKeyWordGap) + 1)
	#print(l_getSearchEndPosRow[l_One])
	#print(l_getReadingEndPosRow)
	
	# Get actual values in list
	l_DialCodePosVal = Utility.getConfValueUsingLevel('l_DialCodePos','APELBY')
	#print(l_DialCodePosVal)
	...
	#print(l_DialCodePosList)
	#print(len(l_DialCodePosList))
	
	l_CheckFirstSepratorVal = Utility.getConfValueUsingLevel('l_CheckFirstSeprator','APELBY')
	l_CheckSecondSepratorVal = Utility.getConfValueUsingLevel('l_CheckSecondSeprator','APELBY')
	#print(l_CheckFirstSepratorVal)
	#print(l_CheckSecondSepratorVal)
	
	if Utility.listToCheckSymbolPresentOrNot(l_DialCodePosList,l_CheckFirstSepratorVal) == l_One:
		l_loopVar = l_loopVar + l_One
		
	if Utility.listToCheckSymbolPresentOrNot(l_DialCodePosList,l_CheckSecondSepratorVal) == l_One:
		l_loopVar = l_loopVar + l_One
	#print(l_loopVar)
	
	l_EffectiveEndDateVal =  Utility.getConfValueUsingLevel('l_EffectiveEndDate','APELBY')
	l_EffectiveEndDateValDate = UtilityForExcelFile.getConvertNumberToDateInDDMMYYYYHH24MISS(l_EffectiveEndDateVal)
	
	l_FetchColumnSequence1Val = Utility.getConfValueUsingLevel('l_FetchColumnSequence1','APELBY')
	l_FetchColumnSequence2Val = Utility.getConfValueUsingLevel('l_FetchColumnSequence2','APELBY')
	l_FetchColumnSequence3Val = Utility.getConfValueUsingLevel('l_FetchColumnSequence3','APELBY')
	l_FetchColumnSequence4Val = Utility.getConfValueUsingLevel('l_FetchColumnSequence4','APELBY')
	l_FetchColumnSequence5Val = Utility.getConfValueUsingLevel('l_FetchColumnSequence5','APELBY')
	...
	l_PtrnerNameSl					= str(Utility.getConfValueUsingLevel('l_PtrnerName','APELBY'))
	l_destinationTypeSl				= str(Utility.getConfValueUsingLevel('destination_Type'))
	
	# Read Excel Sheet and Updating into Data frame.
	excel_data_df = pandas.read_excel(p_readFilePath + p_file, sheet_name = int(l_getCanarSheetName), header = int(l_getRadingStartPosRow), usecols = w_l)
	...
	l_dropFromTailOfDataFreamVal	= str(Utility.getConfValueUsingLevel('l_dropFromTailOfDataFream','APELBY'))
	...
	l_PresentDifferSeparatorInUnitRateVal	= str(Utility.getConfValueUsingLevel('l_PresentDifferSeparatorInUnitRate','APELBY'))
	if int(l_PresentDifferSeparatorInUnitRateVal) == l_One:
		l_PresentSeparatorInUnitRateVal	= str(Utility.getConfValueUsingLevel('l_PresentSeparatorInUnitRate','APELBY'))
