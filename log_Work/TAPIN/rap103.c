 ...
 *    rap103.c
 *
 *    "RAP-0103" ASN.1 module encode/decode/print/free C src.
 *
 *    This file was generated by snacc on Thu Dec  1 16:13:44 2005
 *
...
AsnLen
BEncReturnSummaryContent PARAMS ((b, v),
BUF_TYPE b _AND_
ReturnSummary *v)
{
    AsnLen totalLen = 0;
    ...
    if (NOT_NULL ((v->severeReturnValue)))
    {
    itemLen = BEncSevereReturnValueContent (b, (v->severeReturnValue));
    BEncDefLenTo127 (b, itemLen);
    itemLen++;
    itemLen += BEncTag3 (b, APPL, PRIM, 529);
...
}  /* BEncReturnSummaryContent */

void
BDecReturnSummaryContent PARAMS ((b, tagId0, elmtLen0, v, bytesDecoded, env),
BUF_TYPE b _AND_
AsnTag tagId0 _AND_
AsnLen elmtLen0 _AND_
ReturnSummary *v _AND_
AsnLen *bytesDecoded _AND_
ENV_TYPE env)
{
    int seqDone = FALSE;
...
(tagId1 == MAKE_TAG_ID (APPL, CONS, 109))))
    ...
    if ((!seqDone) && ((tagId1 == MAKE_TAG_ID (APPL, PRIM, 529))))
    {
    elmtLen1 = BDecLen (b, &totalElmtsLen1, env);
    (v->severeReturnValue) = (SevereReturnValue*) Asn1Alloc (sizeof (SevereReturnValue));
    CheckAsn1Alloc ((v->severeReturnValue), env);
    BDecSevereReturnValueContent (b, tagId1, elmtLen1, (v->severeReturnValue), &totalElmtsLen1, env);
        seqDone = TRUE;
        if (elmtLen0 == INDEFINITE_LEN)
            BDecEoc (b, &totalElmtsLen1, env);
        else if (totalElmtsLen1 != elmtLen0)
    ...
    (*bytesDecoded) += totalElmtsLen1;
}  /* BDecReturnSummaryContent */

void
PrintReturnSummary PARAMS ((f, v, indent),
FILE* f _AND_
ReturnSummary *v _AND_
unsigned short int indent)
{
    ...
    fprintf (f,",\n");
    Indent (f, indent + stdIndentG);
    fprintf (f,"severeReturnValue ");
    PrintSevereReturnValue (f, (v->severeReturnValue), indent + stdIndentG);
    }
    fprintf (f,"\n");
    Indent (f, indent);
    fprintf (f,"}");
...
ReturnSummary *v)
{
    ...
    if (NOT_NULL ((v->severeReturnValue)))
    {
    FreeSevereReturnValue ((v->severeReturnValue));
    Asn1Free ((v->severeReturnValue));
    }
...
AsnLen
BEncReturnSummaryListContent PARAMS ((b, v),
BUF_TYPE b _AND_
ReturnSummaryList *v)
{
    AsnLen totalLen = 0;
    ...
    {
    BEncEocIfNec (b);
    itemLen = BEncReturnSummaryContent (b, component);
    itemLen += BEncConsLen (b, itemLen);
    itemLen += BEncTag3 (b, APPL, CONS, 530);

        listLen += itemLen;
    }
    return listLen;

}  /* BEncReturnSummaryListContent */

void
BDecReturnSummaryListContent PARAMS ((b, tagId0, elmtLen0, v, bytesDecoded, env),
BUF_TYPE b _AND_
AsnTag tagId0 _AND_
AsnLen elmtLen0 _AND_
ReturnSummaryList *v _AND_
AsnLen *bytesDecoded _AND_
ENV_TYPE env)
{
    int seqDone = FALSE;
    ...
    if ((tagId1 == MAKE_TAG_ID (APPL, CONS, 530)))
    {
        elmtLen1 = BDecLen (b, &totalElmtsLen1, env);
    tmpVar = (ReturnSummary**) AsnListAppend (v);
    (*tmpVar) = (ReturnSummary*) Asn1Alloc (sizeof (ReturnSummary));
    CheckAsn1Alloc ((*tmpVar), env);
    BDecReturnSummaryContent (b, tagId1, elmtLen1, (*tmpVar), &totalElmtsLen1, env);
    }  /* end of tag check if */
    else  /* wrong tag */
    {
         Asn1Error ("Unexpected Tag\n");
    ...
    (*bytesDecoded) += totalElmtsLen1;
}  /* BDecReturnSummaryListContent */

void
PrintReturnSummaryList PARAMS ((f, v, indent),
FILE* f _AND_
ReturnSummaryList *v _AND_
unsigned short int indent)
{
    ...
    {
        Indent (f, indent+ stdIndentG);
        PrintReturnSummary (f, tmp, indent + stdIndentG);
        if (tmp != (ReturnSummary*)LAST_LIST_ELMT (v))
            fprintf (f,",\n");
    }
    fprintf (f,"\n");
    Indent (f, indent);
    fprintf (f,"}");
}  /* PrintReturnSummaryList */

void
FreeReturnSummaryList PARAMS ((v),
ReturnSummaryList *v)
{
    ...
    for (l = FIRST_LIST_NODE (v); l != NULL; )
    {
        FreeReturnSummary ((l->data));
        tmp = l->next;
        Asn1Free (l->data);
        Asn1Free (l);
        l = tmp;
    }
}  /* FreeReturnSummaryList */
...
RapAuditControlInfo *v)
{
    ...
    if (NOT_NULL ((v->returnSummaryList)))
    {
    BEncEocIfNec (b);
    itemLen = BEncReturnSummaryListContent (b, (v->returnSummaryList));
    itemLen += BEncConsLen (b, itemLen);
    itemLen += BEncTag3 (b, APPL, CONS, 531);
...
ENV_TYPE env)
{
    ...
    if (((tagId1 == MAKE_TAG_ID (APPL, CONS, 531))))
    {
    elmtLen1 = BDecLen (b, &totalElmtsLen1, env);
    (v->returnSummaryList) = AsnListNew (sizeof (char*));
    CheckAsn1Alloc ((v->returnSummaryList), env);
    BDecReturnSummaryListContent (b, tagId1, elmtLen1, (v->returnSummaryList), &totalElmtsLen1, env);
    tagId1 = BDecTag (b, &totalElmtsLen1, env);
    }
...
unsigned short int indent)
{
    ...
    {
    Indent (f, indent + stdIndentG);
    fprintf (f,"returnSummaryList ");
    PrintReturnSummaryList (f, (v->returnSummaryList), indent + stdIndentG);
    fprintf (f, ",\n");
    }
    Indent (f, indent + stdIndentG);
...
RapAuditControlInfo *v)
{
    ...
    if (NOT_NULL ((v->returnSummaryList)))
    {
    FreeReturnSummaryList ((v->returnSummaryList));
    Asn1Free ((v->returnSummaryList));
    }
